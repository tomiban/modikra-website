	...
	if (isCatalogoPage) {
		return (
			<header className={`fixed left-0 top-0 md:shadow-none z-50 w-full`}>
				<div
					className={`md:px-10 py-2 transition-all duration-300 ease-in bg-gradient-to-l from-gray-700 to-gray-900 dark:bg-gray-800`}>
					<div className='flex items-center justify-between px-4 sm:px-0'>
						...
						<h3 className='flex items-center tracking-wide font-bold text-white text-xl justify-center ml-4'>
							<svg
						...
						<Button
							...
							color={"white"}
							span={
								"hidden md:block md:absolute md:inset-y-0 md:left-0 md:w-[2px] md:bg-white md:transition-all md:group-hover:w-full md:group-active:bg-white-500"
							}
						/>
					</div>
				</div>
				...
				<div
					...
					<div className='flex items-center justify-between cursor-pointer w-full relative'>
						<Link
							...
							<span className='sr-only'>Inicio</span>
							{screenX ? (
								<LogoVertical fill={"#273349"} className='block' />
							) : (
								<LogoHorizontal
									fill={"#273349"}
